...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (status === "complete") {
      ...
      const badgesString = badges.length > 0 ? badges.join(",") : null; // Store as NULL if empty
      ...
      if (existingReward.length > 0) {
        await pool.query(
          `UPDATE user_rewards 
           SET points = ?, highest_streak = ?, badges = ? 
           WHERE user_id = ? AND habit_id = ?`,
          [totalPoints, highestStreak, badgesString, user_id, habit_id]
        );
      } else {
        await pool.query(
          `INSERT INTO user_rewards (user_id, habit_id, points, highest_streak, badges) 
           VALUES (?, ?, ?, ?, ?)`,
          [user_id, habit_id, totalPoints, highestStreak, badgesString]
        );
      }
