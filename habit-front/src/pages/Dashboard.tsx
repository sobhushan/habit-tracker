...
useEffect(() => {
  ...
  const handleToggleComplete = async (habit_id: number, status: string) => {
    ...
    const completion_date = new Date().toISOString().split("T")[0]; // Format: YYYY-MM-DD
          ...
          <div className="col-md-8 mt-4 mb-4">
              ...
              <div className="row">
                ...
                <div key={habit.habit_id} className="col-12 mb-3">
                  <div 
                    className="card p-3"
                    style={{ background: habit.status === "Completed" ? "linear-gradient(225deg, #28a745, #d4edda)" : "linear-gradient(225deg, #dc3545, #f8d7da)", color: "#000", cursor: "pointer" }}
