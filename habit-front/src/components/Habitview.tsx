...
//============================================================
import { useState, useEffect } from "react";
import { format, startOfMonth, endOfMonth, eachDayOfInterval, getDay, isSameDay, isAfter, startOfWeek, endOfWeek } from "date-fns";
import axios from "axios";
...
const Habitview: React.FC<Props> = ({ value = new Date(), onChange, user_id, viewMode }) => {
  ...
  // const firstDay = startOfMonth(value);
  // const lastDay = endOfMonth(value);
  const firstDay = viewMode === "Month" ? startOfMonth(value) : startOfWeek(value);
  const lastDay = viewMode === "Month" ? endOfMonth(value) : endOfWeek(value);
  const daysToShow = eachDayOfInterval({ start: firstDay, end: lastDay });
  ...
  return (
    <div className="w-full flex flex-col items-center"
      ...
      <div className="w-full max-w-5xl border p-4">
        ...
        {/*<div className="grid grid-cols-7 gap-2">
          ...
          {daysInMonth.map((day) => {
            ...
            return (
              <div
                key={day.toString()}
                className={`p-2 border rounded cursor-pointer ${isActive ? 'bg-pink-700 text-white' : 'bg-white'}`}
                // onClick={() => onChange(day)}
        ...
        <div className="grid grid-cols-7 gap-2">
          {viewMode === "Month" && emptySlotsStart.map((_, i) => <div key={`empty-start-${i}`} />)}
